				...
				<form action="${pageContext.request.contextPath}/workers" method="post">
					<table style="with: 50%" border = "1">
						...
						<c:forEach items="${listWorkers}" var="worker">
							...
							<td><button type="submit" name="Delete" value=${worker.workerid}>Delete</button></td>
							<td><button type="submit" name="Update" value=${worker.workerid}>Update</button></td>
						</tr>
						</c:forEach>
						<tr>
                        ...
                        <tr>
                            <ul class="pagination">
                                <c:if test="${currentPage != 1}">
                        	        <li><a href="<%=request.getContextPath()%>//workers?page=${currentPage - 1}">Previous</a></li>
                                </c:if>
                                <c:forEach begin="1" end="${noOfPages}" var="i">
                                    <c:choose>
                                        <c:when test="${currentPage eq i}">
                                            <li><a class="active" href="<%=request.getContextPath()%>//workers?page=${i}">${i}</a></li>
                                        </c:when>
                                        <c:otherwise>
                        	                <li><a href="<%=request.getContextPath()%>//workers?page=${i}">${i}</a></li>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                                <c:if test="${currentPage lt noOfPages}">
                        	        <li><a href="<%=request.getContextPath()%>//workers?page=${currentPage +1}">Next</a></li>
                                </c:if>
                             </u1>
                        </tr>
					</table>

<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<html>
	<head>
        <meta charset="utf-8">
        <title>Аэрофлот</title>
        <style>
            <%@include file='style.css' %>
        </style>
    </head>
	<body>
		<div id="container" style="width:100%">
			<div id="header" style="background-color:#A3FFC2;">
				<%@include file="navigationTop.jsp" %>
			</div>
			<div id="menu" style="background-color:#f1f1f1;height:calc(100vh - 40px);width:200px;float:left;">
				<%@include file="navigationLeft.jsp" %>
			</div>
			<div id="content" style="background-color: #fff;height:200px;width:1000px;float:left;">
			<c:choose>
			<c:when test="${not empty flightUp}">       
			</c:when>
			<c:otherwise>
			<div class="example_content2 ">
				<h3>New Flight</h3>
				<form action="${pageContext.request.contextPath}/flights" method="post">
					<table style="with: 50%" border = "1">
						<tr>
							<td>Airport From</td>
							<td>
								<input list="airports" name="AirportFrom" />
								<datalist id="airports">
									<c:forEach items="${listAirport}" var="airport">
										<option value=${airport.id}> ${airport.name} </option>
									</c:forEach>
								</datalist>
							</td>
						</tr>
							<td>Airport To</td>
							<td>
								<input list="airports" name="AirportTo" />
								<datalist id="airports">
									<c:forEach items="${listAirport}" var="airport">
										<option value=${airport.id}> ${airport.name} </option>
									</c:forEach>
								</datalist>
							</td>
						</tr>
						<tr>
							<td>Date</td>
								<td>
									<input id="datetime" type="datetime-local" name="Date">
								</td>
						<tr>
							<td><button type="submit" name="NewFlightAdd" >Add</button></td>
						</tr>
					</table>
				</form>
			</div>
			</c:otherwise>
			</c:choose>
            </div>
            <div id="footer" class="footer">
       Copyright © 2020 www.aeroflot.by
      </div>
       </div>
 </body>
</html>